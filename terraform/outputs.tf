...
output "instance_id" {
  ...
  value       = aws_instance.bouvet.id
}

output "public_ip" {
  description = "Elastic IP address"
  value       = aws_eip.bouvet.public_ip
}

output "private_ip" {
  description = "Private IP address"
  value       = aws_instance.bouvet.private_ip
}

output "ssh_command" {
  description = "SSH command to connect (Debian uses 'admin' user)"
  value       = "ssh -i ~/.ssh/${var.ssh_key_name}.pem admin@${aws_eip.bouvet.public_ip}"
}
